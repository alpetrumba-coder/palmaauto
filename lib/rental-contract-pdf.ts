  ...
  const content: any[] = [
    ...
    { text: "7. ДОСРОЧНОЕ РАСТОРЖЕНИЕ", style: "h2" },
    {
      text:
        ...
        "7.3. Отмена брони: бесплатная отмена возможна, если Арендатор уведомил Арендодателя не позднее чем за 3 (трое) суток до даты начала аренды (по дате начала брони). Если уведомление об отмене получено позднее указанного срока, удерживается оплата за первые сутки аренды (в соответствии с выбранным вариантом оплаты/условиями брони).",
      style: "small",
      margin: [0, 0, 0, 6],
    },

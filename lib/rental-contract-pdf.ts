...
function buildDocDefinition(input: RentalContractPdfInput) {
  ...
  const content: any[] = [
    ...
    {
      text:
        "ООО «ТДР», именуемое в дальнейшем «Арендодатель», в лице Генерального директора ____________________________________, действующего на основании Устава, с одной стороны, и гражданин(ка) " +
        m.fullName +
        ",\n_________ г.р., паспорт: " +
    ...
    { text: "6. ДЕПОЗИТ", style: "h2" },
    {
      text:
        ...
        "6.3. Если по вине Арендатора произойдёт авария или другой страховой случай, и сумма ущерба не превышает 15 000 рублей, то Арендодатель возвращает Арендатору разницу между суммой ущерба и депозитом.\n" +
        "6.4. Если сумма ущерба больше 50 000 рублей, то Арендатор доплачивает Арендодателю разницу.\n" +
